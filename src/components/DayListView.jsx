...
export default function DayListView({day}) {
    ...
    const {tasks, weekIndex} = useContext(GlobalContext)
    ...
    const doesTaskExists = ()=>{
        tasks.map((data)=>{
            var isInBetween = dayjs(day).isBetween(data.startDate , data.endDate) 
            var isStartDate = day.format("YYYY-MM-DD") === data.startDate
            var isEndDate = day.format("YYYY-MM-DD") === data.endDate
            if(isInBetween === true || isStartDate === true || isEndDate === true){
                addToTasks(data)
            }
        })
    }
